...
export const SignUp = () => {
  ...
  const [isAgreedToPromotions, setIsAgreedToPromotions] = useState(false);

  const isEmailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  const isButtonDisabled = !isEmailValid || !isAgreedToTerms;
  ...
  const handleSignUpClick = (e: React.MouseEvent) => {
    e.preventDefault();
    if (!isButtonDisabled) {
      navigate('/sign-up/password', { state: { email } });
    }
  };
  ...
  return (
    <div className="bg-primary-background">
      ...
      ${email && !isEmailValid ? 'border-red-500' : 'border-signup-border focus:border-white'}`}
                value={email}
            ...
            <div className="flex flex-col gap-2">
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  checked={isAgreedToPromotions}
                  onChange={() => setIsAgreedToPromotions(!isAgreedToPromotions)}
                />
